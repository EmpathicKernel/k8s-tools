...
parser.add_option("--release-name", dest = "releaseName", action = "store", default = "release", help = "Label that maps to helm release name")
...
if options.namespace:
    ...
    logging.error("Namespace not supplied... Exiting...")
    sys.exit()

if options.scaleIn and options.scaleOut:
    logging.error("You cannot specify both a scale-in and scale-out event")
    sys.exit()
else:
...
if not options.helmCheck and not options.replicas:
    logging.error("Replicas must be specified when not running a Helm check")
    sys.exit()
...
GlobalReleaseName = options.releaseName
...
def findReleaseNames(deployments):
    ...
    for item in deployments:
        ...
        release = item.metadata.labels.get(GlobalReleaseName, None)
        logging.debug(f"Deployment {deployment_name} has label {GlobalReleaseName} of {release}")
...
def main():
    ...
    logging.debug(f"Deployment data:\n{deployments}")

    if ScaleIn or ScaleOut:
        answer = input(f"Deployment Data:\n{deployments}\n\nContinue with scaling? [(Y)es/(N)o]:  ")
